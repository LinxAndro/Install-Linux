...
if [ "$first" != 1 ]; then
	if [ ! -f $tarball ]; then
		...
		aarch64)
			archurl="arm64" ; size="79MB" ;;
		arm)
			archurl="armhf" ; size="78MB" ;;
		amd64)
			archurl="amd64" ; size="84MB" ;;
		x86_64)
			archurl="amd64" ; size="84MB" ;;	
		i*86)
			archurl="i386" ; size="85MB" ;;
		x86)
			archurl="i386" ; size="85MB" ;;
		*)
		...
		printf " (${size})..."
		wget -q -O $tarball "https://raw.githubusercontent.com/AndronixApp/AndronixOrigin/master/Rootfs/Parrot/$archurl/parrot-rootfs-$archurl.tar.xz"
		printf " ${g}Done\n${n}"
	fi
	mkdir "$folder"
	cd "$folder"
...
if [ -n "\$(ls -A $HOME/parrot-binds)" ]; then
    for f in $HOME/parrot-binds/*; do
      . \$f
    done
fi
command+=" -b /dev"
...
chmod +x $exe
mv $exe /data/data/com.termux/files/usr/bin/parrot
printf " ${g}Done\n${n}"
...
echo -e "Run it with the command: ${g}parrot${n}"